...
public class BuildIndexTable {
	...
	public void buildTable() {
		...
		try {
			this.putIndex = hbaseTool.getIndexTable(indexTableName);
			hTable = hbaseTool.getTable(BuildIndexTable.THEME_NAME);
			Scan scan = new Scan();
			scan.setCacheBlocks(false);
			...
			for (Result result : resultScanner) {
				...
				for (Cell cell : result.rawCells()) {
					String singleColumn = Bytes.toString(CellUtil.cloneValue(cell));
...
//			hTable.close();
	...
	public void putData(Map<byte[], byte[]> map) {
