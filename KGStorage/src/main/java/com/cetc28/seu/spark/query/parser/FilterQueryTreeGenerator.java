...
import com.cetc28.seu.spark.query.model.filterhbase.JoinerFilterNode;
import com.cetc28.seu.spark.query.model.filterhbase.LocalSimpleFilterNode;
import com.cetc28.seu.sparql.FilterClause;
import com.cetc28.seu.sparql.SelectClause;

public class FilterQueryTreeGenerator extends TreeGenerator{

	public FilterQueryTreeGenerator(){
		
	}
	
    public FilterQueryTreeGenerator(Configuration hbaseconf,JavaSparkContext sc){
	...
	public void generate(SelectClause sClause) {
		...
		// 通过查询语句的中的信息生成查询树
				...
				if (joins == null) {
					...
					LocalSimpleFilterNode leafNode=new LocalSimpleFilterNode(nodeid++, -1, -1, -1, sub, qc, sClause.getFilter());
					...
					if(rootRDF.getPredict().getValue().contains("objects"))
					{
						...
						JoinerFilterNode rootNode = new JoinerFilterNode();
...
//				nodeList.add(child);
			...
			else
			{
				...
				child = new JoinerFilterNode(nodeid++, parent.getId(), childTerm,childRDF,
						predicate[1]);
				nodeList.add(child);
			}
			...
			child = new LocalSimpleFilterNode(nodeid++, -1, -1, parent.getId(), childTerm, qc, filterClause);
			nodeList.add(child);
			reduce(counts, childTerm);
			return child.getId();
		} else {
